...
<body>
	...
	<c:if test="${not empty list}">
		<table style="width: 100%" id="myTable">
			<tr class="header">
				<th>Id</th>
				<c:if test="${isAdmin  or isCourier}">
					<th>Customer</th>
				</c:if>
				<th>Status</th>
				<th>Created</th>
			...
			<c:forEach var="listValue" items="${list}" varStatus="status">
				<tr>
					...
					<td><c:if test="${isUser}">
							...
							<c:if
								test='<%=((Order) pageContext.getAttribute("listValue"))
									.getStatus() == OrderStatus.PROCESSING%>'>
								<a> | </a>
								<a href="/user/cancelOrder?id=${listValue.id}">Request
									Cancellation</a>
							</c:if>
							...
							<c:if
								test='<%=((Order) pageContext.getAttribute("listValue"))
									.getStatus() == OrderStatus.CANCEL_REQUESTED%>'>
								<a> | </a>
								<a href="/admin/cancelOrder?id=${listValue.id}">Cancel</a>
							</c:if>
						</c:if> <c:if test="${isCourier}">
							<c:if
								test='<%=((Order) pageContext.getAttribute("listValue"))
									.getStatus() == OrderStatus.PROCESSING%>'>
								<a href="/courier/completeOrder?id=${listValue.id}">Complete
									Order</a>
							</c:if>
						</c:if></td>
				</tr>
